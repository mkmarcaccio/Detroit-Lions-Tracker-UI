...
import { PreviousPlayersAddEditDialogComponent } from '../previous-players-add-edit-dialog/previous-players-add-edit-dialog.component';

@Component({
  selector: 'app-previous-players',
  templateUrl: './previous-players.component.html',
  styleUrls: ['./previous-players.component.scss']
})
export class PreviousPlayersComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.detroitLionsTrackerService.getPlayers()
      .subscribe(response => {
        ...
        this.players = this.players.filter(player => player.isOnRoster == false)
  ...
  addEditPlayers(input: any = new Player) {
    ...
    const dialogRef = this.dialog.open(PreviousPlayersAddEditDialogComponent, {
      minWidth: "600px",
      height: 'auto',
      disableClose: true,
      data: input
